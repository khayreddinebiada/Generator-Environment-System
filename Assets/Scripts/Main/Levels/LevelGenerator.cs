                            ...
                            if (line[i][j] == levelsAttrsGenerator.attributes[k].character)
                            {
                                if (generateOn == GenerateOn.OnZ)
                                    InstaintiateAsPrefab(levelsAttrsGenerator.attributes[k].piece,
                                        new Vector3(j * objectScale.x, 0, ((from == From.DownToUp)?(line.Length - i - 1) : i) * objectScale.y),
                                        Quaternion.identity, new Vector3(objectScale.x, objectScale.x, objectScale.x),
                                        lineObj.transform);
                                else
                                    InstaintiateAsPrefab(levelsAttrsGenerator.attributes[k].piece,
                                        new Vector3(j * objectScale.x, ((from == From.DownToUp) ? (line.Length - i - 1) : i) * objectScale.y, 0),
                                        Quaternion.identity, new Vector3(objectScale.x, objectScale.x, objectScale.x),
                                        lineObj.transform);
                            }
                        }
